...
let GameComponent = class GameComponent {
    ...
    constructor(socketService, ngZone, router) {
        ...
        this.HandImageSrc = [
            "dotball", "one", "two", "three", "four", "five", "six"
        ];
        this.OppoHandImageSrc = [
            "dotball", "oner", "twor", "threer", "fourr", "fiver", "sixr"
        ];
    ...
    ngOnInit() {
        ...
        this.socketService.Emitter.on(GameEvents.UpdateScore, (data) => {
            ...
            this.ngZone.run(() => {
                ...
                if (this.socketService.CurrentPlayer.playerId == data.currentInnings.batting.playerId ||
        ...
        this.socketService.Emitter.on(INternalEvents.ShowMatchWinLose, (data) => {
            if (data.winnerTeam.player1.playerId == this.socketService.CurrentPlayer.playerId
                || data.winnerTeam.player2.playerId == this.socketService.CurrentPlayer.playerId) {
                this.ngZone.run(() => {
    ...
    UpdateScreen(data) {
        ...
        if (currentBatsmanId == currentPlayerId ||
            ...
            this.ngZone.run(() => {
                this.toggleReady = true;
                this.MessageBoxText = currentBatsmanId == currentPlayerId ?
                    `You are on Strike!` : `You are bowling`;
            });
        }
        else {
